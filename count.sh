...
echo "被标注的先后修概念数——计算机" $(jq .c2 prerequisites/cs.json | sort | uniq | wc -l) &
echo "被标注的先后修概念数——数学" $(jq .c2 prerequisites/math.json | sort | uniq | wc -l) &
echo "被标注的先后修概念数——心理学" $(jq .c2 prerequisites/psy.json | sort | uniq | wc -l) &
